    ...
    <nav className="w-full bg-deepBlue text-white py-5 px-6 md:px-12 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="bg-brightOrange text-deepBlue font-semibold px-4 py-2 rounded-md shadow hover:bg-orange-400"
        >
          Get Started
        </button>
      </div>
    ...
    <header className="min-h-[60vh] flex items-center justify-center text-center px-6 md:px-12 bg-deepBlue">
      <div className="max-w-4xl">
        ...
        <p className="mt-6 text-lg text-white/80">
          Spectrum Arena helps you find artisans, post jobs, and manage savings — all in one connected ecosystem.
        </p>
        <div className="mt-10">
          <button
            onClick={onOpenSignup}
            className="bg-brightOrange text-deepBlue font-bold px-8 py-3 rounded-lg shadow hover:bg-orange-400 transition-all duration-200"
          >
            Get Started
          </button>
        </div>
    ...
    <section id="how" className="py-16 px-6 md:px-12 bg-deepBlue text-white">
      ...
      <div className="mt-10 grid grid-cols-1 md:grid-cols-4 gap-6">
        {steps.map((title, i) => (
          <div
            key={i}
            className="bg-deepBlue/80 p-6 rounded-xl shadow-inner min-h-[160px] flex items-center justify-center border border-white/10"
          >
...
function SignupModal({ onClose, onOTPRequest }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      onOTPRequest(phone);
      onClose();
      navigate("/choice"); // ✅ Redirect to choice screen
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#0d0f14] text-white rounded-xl p-6 w-full max-w-md relative">
        ...
        <form onSubmit={submit} className="flex flex-col gap-3">
          <input required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Full Name" className="border border-gray-500 rounded-md p-2 bg-[#0d0f14]" />
          <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email (optional)" className="border border-gray-500 rounded-md p-2 bg-[#0d0f14]" />
          <input required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone Number" className="border border-gray-500 rounded-md p-2 bg-[#0d0f14]" />
          <input required value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Password" className="border border-gray-500 rounded-md p-2 bg-[#0d0f14]" />
          {error && <div className="text-red-500 text-sm">{error}</div>}
          <button type="submit" disabled={loading} className="mt-3 bg-brightOrange text-deepBlue font-semibold py-2 rounded-md hover:bg-orange-400 transition-all duration-300">
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#0d0f14] text-white rounded-xl p-6 w-full max-w-md relative">
        ...
        <form onSubmit={submit} className="flex flex-col gap-3">
          <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone Number" className="border border-gray-500 rounded-md p-2 bg-[#0d0f14]" />
          <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Password" className="border border-gray-500 rounded-md p-2 bg-[#0d0f14]" />
          {error && <div className="text-red-500 text-sm">{error}</div>}
          <button type="submit" disabled={loading} className="mt-3 bg-brightOrange text-deepBlue font-semibold py-2 rounded-md hover:bg-orange-400 transition-all duration-300">
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/post-job" element={<PostJob />} /> {/* ✅ NEW */}
      </Routes>
    </Router>
  );
}
