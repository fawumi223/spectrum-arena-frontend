...
export default function Data() {
  ...
  const submitData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/bills/data/`, {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        setMessage("Data purchase request submitted! (Pending Providus Gateway)");
      } else {
        setMessage(data.error || "Unable to submit data purchase request.");
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-deepBlue text-white px-6 py-10">
      ...
      <div className="bg-[#111827] p-6 rounded-xl border border-white/10 shadow-md max-w-md mx-auto">
        ...
        {/* Data Plan */}
        <label className="text-sm text-white/70">Select Data Plan</label>
        <select
          value={plan}
        ...
        >
          ...
          {/* Basic Sample Plans — replace with Providus plans later */}
          <option value="500mb">₦150 — 500MB</option>
          <option value="1gb">₦300 — 1GB</option>
          <option value="2gb">₦500 — 2GB</option>
          <option value="5gb">₦1200 — 5GB</option>
          <option value="10gb">₦2200 — 10GB</option>
        </select>
